...
/**
 * Clase de utilidades para el parqueo con métodos estáticos.
 * Contiene funciones de validación y cálculo de costos.
 * @author David Euceda
 */
public class UtilidadesParqueo {
    ...
    /**
     * Calcula el costo del estacionamiento según el tiempo en minutos
     * @param minutos tiempo estacionado
     * @return costo en lempiras
     */
    public static int calcularCosto(int minutos) {
        if (minutos <= 0) return 0;
        if (minutos <= 180) return 10;
        else if (minutos <= 240) return 20;
        else if (minutos <= 300) return 30;
        else if (minutos <= 360) return 40;
        else return 100;
    }
}
